...
- **爬虫:** OpenCLI → CDP → Playwright (降级链)
- **热门话题:** AutoCLI (Chrome CDP 复用登录态，支持 16 个平台)
...
backend/
  ...
  crawlers/         # 爬虫实现 (router, opencli, cdp, claude, playwright base, x, youtube, xiaohongshu, douyin, weibo, toutiao, tiantai108)
  data/             # SQLite 数据库文件
frontend/
  src/pages/        # 页面组件 (Dashboard, SocialAccounts, Websites, Settings, Login, MonitorDetail, HotTopicsPage)
...
OpenCLI → CDP → Playwright，通过 `crawlers/router.py:CrawlerRouter` 统一调度。
...
- **AutoCLI 依赖 Chrome**：`browser` 模式平台需要 Chrome 运行且开启 CDP 调试端口，否则抓取失败
- **舆情搜索图片显示**：微博图片需通过 `/api/tools/proxy/image` 代理，否则 403
- **微博 m.weibo.cn 页面持久连接**: `page.goto` 使用 `domcontentloaded` 而非 `networkidle`，否则超时
