...
async function discoverChromePort() {
  ...
  if (fallback !== null) {
    const fbPort = typeof fallback === 'object' ? fallback.port : fallback;
    const fbWsPath = typeof fallback === 'object' ? fallback.wsPath : null;
    connectedBrowser = { id: 'unknown', label: '未知（通过手动调试端口连接）', source: 'fallback' };
    console.log(`[CDP Proxy] 通过手动调试端口连接: ${fbPort}${fbWsPath ? ` (wsPath: ${fbWsPath})` : ''}`);
    return { port: fbPort, wsPath: fbWsPath };
  }
  return null;
}
