...
// 兜底：扫描常用固定端口，自动发现 wsPath
// 适用场景：用户手动 --remote-debugging-port=9222 启动浏览器，
// 此时 DevToolsActivePort 可能不在默认 user-data-dir。
// Chrome 149+ 要求 WebSocket URL 包含浏览器 GUID 路径，
// 因此需要从 /json/version 获取完整的 webSocketDebuggerUrl。
export async function findFallbackPort() {
  for (const port of [9222, 9229, 9333]) {
    if (await checkPort(port)) {
      // Try to discover the browser GUID wsPath from /json/version
      let wsPath = null;
      try {
        ...
        if (resp.ok) {
          ...
          const m = wsUrl.match(/^ws:\/\/[^/]+(\/devtools\/browser\/[^\s]*)$/);
          if (m) wsPath = m[1];
        }
      } catch { /* keep wsPath null */ }
      return { port, wsPath };
    }
  }
  return null;
